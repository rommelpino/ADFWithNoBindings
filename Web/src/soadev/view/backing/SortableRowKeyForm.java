...
import soadev.ext.trinidad.model.SortableModel;

public class SortableRowKeyForm {
    ...
    public CollectionModel getModel() {
        if (model == null) {
            ...
            model = new SortableModel(getJobList());
        }
        return model;
    }
    ...
    public static void handleFilter(QueryEvent event) {
        ...
        SortableModel colModel =
            (SortableModel)table.getValue();

        FilterableQueryDescriptor descriptor =
            (FilterableQueryDescriptor)event.getDescriptor();
        String expression = FilterableQueryDescriptorGroovyParser.parseGroovyExpression("o", descriptor);
        System.out.println(expression);
        List sortedList = colModel.getSortedList();
        List<SortableModel.Row> wrappedData = (List<SortableModel.Row>)colModel.getWrappedData();
        sortedList.clear();

        try {
            for (SortableModel.Row row: wrappedData) {
                Object obj = row.getDataProvider();
                if (expression == null || "".equals(expression)) {
                    sortedList.add(row.getKey());
                } else {
